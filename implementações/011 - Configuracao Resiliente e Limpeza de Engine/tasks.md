...
### [x] T-001 — Mapear consumidores e confirmar findings no código
...
### [x] T-002 — Preparar fixtures de teste (YAML válido e malformado)
...
- **Arquivos envolvidos:** `tests/test_config_resilience.py` (usa `tmp_path` do pytest em vez de fixtures fixas).
...
### [x] T-003 — Carregamento atômico e degradação em `_load_configs`/`reload` (CO-02, CO-03)
...
- **Arquivos envolvidos:** `src/infrastructure/config/config_service.py` (`_load_configs` 100-117, `reload` 119-130).
...
### [x] T-004 — Tornar `get_calendar_id` resiliente a valor não-string (CO-06)
...
- **Arquivos envolvidos:** `src/infrastructure/config/config_service.py` (`get_calendar_id` 346-352).
- **Critério de conclusão:** Atende RF-003; nenhuma `AttributeError` possível.
- **Dependências:** T-001.
- **Estimativa:** Pequena.

### [x] T-005 — Fallback não-vazio e log em `get_message` (CO-07)
...
- **Arquivos envolvidos:** `src/infrastructure/config/config_service.py` (`get_message` 294-318, `_get_fallback_message` 320-329).
- **Critério de conclusão:** Atende RF-004; paciente nunca recebe mensagem em branco; log gerado.
- **Dependências:** T-001.
- **Estimativa:** Média.

### [x] T-006 — Resolver `${VAR}` no meio da string e não vazar literal (CO-09)
...
- **Arquivos envolvidos:** `src/infrastructure/config/config_service.py` (`_resolve_env_vars` 83-98).
- **Critério de conclusão:** Atende RF-005; literal `${...}` nunca chega ao consumidor; interpolação no meio de texto funciona.
- **Dependências:** T-001.
- **Estimativa:** Média.

### [x] T-007 — Lock do singleton e default não-truncado (CO-10)
...
- **Arquivos envolvidos:** `src/infrastructure/config/config_service.py` (`__new__` 37-43, `get_doctor_name` 331-333, `_instance_lock` 21).
- **Critério de conclusão:** Atende RF-006; `_load_configs` chamado uma vez sob corrida; default sem truncamento.
- **Dependências:** T-001.
- **Estimativa:** Pequena.

### [x] T-008 — Remover dependência e tags de engine mortas (EN-04)
...
### [x] T-009 — Testes unitários de configuração resiliente (CO-02/03/06/07/09/10)
...
- **Arquivos envolvidos:** `tests/test_config_resilience.py` (novo).
- **Critério de conclusão:** Cobre CA-001 a CA-007; todos verdes.
- **Dependências:** T-003, T-004, T-005, T-006, T-007.
- **Estimativa:** Grande.

### [x] T-010 — Testes de casos de borda de configuração
...
- **Arquivos envolvidos:** `tests/test_config_resilience.py`.
- **Critério de conclusão:** Todos os casos de borda passam sem exceção e sem retorno vazio.
- **Dependências:** T-003, T-004, T-005, T-006, T-007.
- **Estimativa:** Média.

### [x] T-011 — Testes de integração de startup e ausência de langgraph
- **Descrição:** Verificar que suíte completa passa após remoção de `langgraph` (259 testes verdes), confirmando que `setup_logging` e o import de `app` funcionam.
- **Arquivos envolvidos:** Suíte completa via `pytest -q`.
- **Critério de conclusão:** Cobre CA-009, CA-010; 259 testes verdes.
...
### [x] T-012 — Corrigir documentação do engine fantasma (EN-02, EN-03)
...
|---|---|---|---|---|
| T-001 | Preparação | [x] 2026-06-15 | Pequena | (todos — investigação) |
| T-002 | Preparação | [x] 2026-06-15 | Média | CO-02/03/06/07/09 |
| T-003 | Implementação | [x] 2026-06-15 | Média | CO-02, CO-03 |
| T-004 | Implementação | [x] 2026-06-15 | Pequena | CO-06 |
| T-005 | Implementação | [x] 2026-06-15 | Média | CO-07 |
| T-006 | Implementação | [x] 2026-06-15 | Média | CO-09 |
| T-007 | Implementação | [x] 2026-06-15 | Pequena | CO-10 |
| T-008 | Implementação | [x] 2026-06-15 | Média | EN-04 |
| T-009 | Testes | [x] 2026-06-15 | Grande | CO-02/03/06/07/09/10 |
| T-010 | Testes | [x] 2026-06-15 | Média | CO-02/06/07/09 (borda) |
| T-011 | Testes | [x] 2026-06-15 | Média | CO-03, EN-04 |
| T-012 | Documentação | [x] 2026-06-15 | Pequena | EN-02, EN-03 |

**Total:** 12 tarefas | **Concluídas:** 12 | **Progresso:** 100%
